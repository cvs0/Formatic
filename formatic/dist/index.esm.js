...
function Text(initialValue, validate, useDebounce, debounceWait) {
    ...
    var validateValue = useCallback(function (newValue) {
        if (validate) {
            setError(validate(newValue));
        }
    }, [validate]);
    var debouncedValidate = useCallback(debounce(function (newValue) {
...
function List(spec, initialState) {
    ...
    var _a = useState(initialState), items = _a[0], setItems = _a[1];
